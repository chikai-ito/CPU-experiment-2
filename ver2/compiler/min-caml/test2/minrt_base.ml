...
let rec read_screen_settings _ =
  ...
  (* スクリーン中心の座標 *)
  screen.(0) <- 0.; (* read_float (); *)
  screen.(1) <- 0.; (* read_float (); *)
  screen.(2) <- 0.; (* read_float (); *)
  (* 回転角 *)
  let v1 = rad 0.(* (read_float ()) *) in
  let cos_v1 = cos v1 in
  let sin_v1 = sin v1 in
  let v2 = rad 30. (* (read_float ()) *) in
...
let rec read_light _ =
  ...
  let nl = 1 (* read_int () *) in
  (* 光線関係 *)
  let l1 = rad 0. (* (read_float ()) *) in
  let sl1 = sin l1 in
  light.(1) <- fneg sl1;
  let l2 = rad 0. (* (read_float ()) *) in
  let cl1 = cos l1 in
  let sl2 = sin l2 in
  light.(0) <- cl1 *. sl2;
  let cl2 = cos l2 in
  light.(2) <- cl1 *. cl2;
  beam.(0) <- 255. (* read_float () *)
...
(**** オブジェクト1つのデータの読み込み ****)
let rec read_nth_object _ =

  let texture = 0 (* read_int () *) in
  if texture <> -1 then
    (
      let form = 1 (* read_int () *) in
      let refltype = 2 (* read_int () *) in
      let isrot_p = 0 (* read_int () *) in

      let abc = create_array 3 0.0 in
      abc.(0) <- 40.; (* read_float (); *)
      abc.(1) <- 10.; (* read_float (); (* 5 *) *)
      abc.(2) <- 40.; (* read_float (); *)
      ...
      let m_invert = fisneg 1. (* (read_float ()) *) in (* 10 *)

      let reflparam = create_array 2 0.0 in
      reflparam.(0) <- 0.2; (* read_float (); *) (* diffuse *)
      reflparam.(1) <- 64.; (* read_float (); *) (* hilight *)

      let color = create_array 3 0.0 in
      color.(0) <- 255.; (* read_float (); *)
      color.(1) <- 255.; (* read_float (); *)
      color.(2) <- 0.; (* read_float (); (* 15 *) *)
...
let rec read_nth_object2 _ =

  let texture = 4 (* read_int () *) in
  if texture <> -1 then
    (
      let form = 3 (* read_int () *) in
      let refltype = 1 (* read_int () *) in
      let isrot_p = 0 (* read_int () *) in

      let abc = create_array 3 0.0 in
      abc.(0) <- 30.; (* read_float (); *)
      abc.(1) <- 30.; (* read_float (); (* 5 *) *)
      abc.(2) <- 30.; (* read_float (); *)
      ...
      let reflparam = create_array 2 0.0 in
      reflparam.(0) <- 1.; (* read_float (); (* diffuse *) *)
      reflparam.(1) <- 255.; (* read_float (); (* hilight *) *)

      let color = create_array 3 0.0 in
      color.(0) <- 255.; (* read_float (); *)
      color.(1) <- 255.; (* read_float (); *)
      color.(2) <- 255.; (* read_float (); (* 15 *) *)
...
let rec read_net_item2 _ =
  let v = create_array 2 (-1) in
  v.(0) <- 1;
  v
in

let rec read_net_item3 _ =
  let v = create_array 4 (-1) in
  v.(0) <- 99;
  v.(1) <- 0;
  v.(2) <- 1;
  v
...
let rec read_or_network _ =
  let a = create_array 1 (-1) in
  let v = create_array 2 a in
  v.(0) <- read_net_item3 ();
  v
...
let rec read_and_network _ =
  and_net.(0) <- read_net_item ();
  and_net.(1) <- read_net_item2 ()
  (* let net = read_net_item 0 in
  if net.(0) = -1 then ()
  else (
    and_net.(n) <- net;
