...
let rec g env = function (* 命令列の13bit即値最適化 (caml2html: simm13_g) *)
  ...
  | Let((x, t), Set(i), e) when -4096 <= i && i < 4096 ->
      ...
      ((* Format.eprintf "erased redundant Set to %s@." x; *)
       e')
  | Let(xt, SLLI(y, C(i)), e) when M.mem y env -> (* for array access *)
...
and g' env = function (* 各命令の13bit即値最適化 (caml2html: simm13_gprime) *)
  | Add(x, y) when M.mem y env -> AddI(x, C(M.find y env))
  | Add(x, y) when M.mem x env -> AddI(y, C(M.find x env))
  (* | Sub(x, V(y)) when M.mem y env -> Sub(x, C(M.find y env)) *)
  | SLL(x, y) when M.mem y env -> SLLI(x, C(M.find y env)) 
   ...
   * | IfGt(x, V(y), e1, e2) when M.mem y env -> IfGt(x, C(M.find y env), g env e1, g env e2)
   ...
   * | IfLt(x, V(y), e1, e2) when M.mem x env -> IfGt(y, C(M.find x env), g env e1, g env e2)
   * | IfGE(x, V(y), e1, e2) when M.mem x env -> IfLE(y, C(M.find x env), g env e1, g env e2)
   * | IfGt(x, V(y), e1, e2) when M.mem x env -> IfLt(y, C(M.find x env), g env e1, g env e2)
   * | IfEq(x, y', e1, e2) -> IfEq(x, y', g env e1, g env e2)
   * | IfLE(x, y', e1, e2) -> IfLE(x, y', g env e1, g env e2)
   * | IfLt(x, y', e1, e2) -> IfLt(x, y', g env e1, g env e2)
   * | IfGE(x, y', e1, e2) -> IfGE(x, y', g env e1, g env e2)
   * | IfGt(x, y', e1, e2) -> IfGt(x, y', g env e1, g env e2)
