...
and g' env = function (* 各命令の13bit即値最適化 (caml2html: simm13_gprime) *)
  ...
  | Ld(mem, x, V(y)) when M.mem y env -> Ld(mem, x, C(M.find y env))
  | St(mem, x, y, V(z)) when M.mem z env -> St(mem, x, y, C(M.find z env))
  | LdF(mem, x, V(y)) when M.mem y env -> LdF(mem, x, C(M.find y env))
  | StF(mem, x, y, V(z)) when M.mem z env -> StF(mem, x, y, C(M.find z env))
  | e -> e
